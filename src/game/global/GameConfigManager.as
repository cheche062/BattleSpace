...
//		public static var card_param_json:Object;	//抽卡配置
		 ...
		 * 周卡表
		 */
		public static var WeekbCardObj:Object = { };
		...
		{
			WeekbCardObj=ResourceManager.instance.getResByURL("config/card_privilege.json");
...
//			skill_control_dic;
		...
		public function loaderArmyGroup():void
		{
			...
			ArmyGroupSeasonVec = ResourceManager.instance.getResByURL("config/juntuan/juntuan_season.json");
//			if (js)
//			{
...
//			var len:int = ArmyGroupSeasonVec.length;
			...
			for (var id in ArmyGroupSeasonVec) {
				var tr:Array = [];
				var reArr:Array = ArmyGroupSeasonVec[id].season_reward.split(",");
				var l1:int = reArr.length;
				ArmyGroupSeasonReward[id] = { };
				for (var j:int = 0; j < l1; j++) 
				{
					...
					ArmyGroupSeasonReward[id][randID] = { rank:randID, reward:realRe };
				}
			}
...
//			if (!_pvpRewardVos)
//			{
//				var _json:*=ResourceManager.instance.getResByURL("config/pvp_rank_reward.json");
//				if (_json)
//				{
//					_pvpRewardVos=[];
//					for each (var c:* in _json)
...
//						vo.num = ItemManager.StringToReward(c.reward)[0].inum;
//						_pvpRewardVos.push(vo);
//					}
//				}
//				else
//				{
//					trace("配置未加载:config/pvp_rank_reward.json");
//				}
//			}
			return _pvpRewardVos;
		}
		
		private static var _pvpIntegralVos:Array;
		
		public static function get pvpIntegralVos():Array
		{
			if (!_pvpIntegralVos)
			{
				var _json:*=ResourceManager.instance.getResByURL("config/pvp_integral_reward.json");
				if (_json)
				{
					_pvpIntegralVos=[];
					for each (var c:* in _json)
