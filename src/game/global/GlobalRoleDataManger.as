package game.global
{
	...
	public class GlobalRoleDataManger
	{
		...
		private function onResult(cmd:int, ... args):void
		{
			...
			switch (cmd)
			{
				case ServiceConst.GET_WATER:
					...
					{
						var diss = parseInt(args[0]) - GlobalRoleDataManger.instance.user.water; 
						// 水变多了说明充值了
						if (diss > 0) {
							var text = GameLanguage.getLangByKey("L_A_178").replace("{0}", diss);
							XAlert.showAlert(text, null, null, true, false);
						}
		...
		public function loginWin(... args):void
		{
			...
			if (args[1].pop_hotevents)
			{
				this.user.pop_hotevents=args[1].pop_hotevents;
			}
...
//							} );
			...
			Signal.intance.on(ServiceConst.getServerEventKey(ServiceConst.BOSS_SERVER_FOOD_LESS), this, onBossNoFood);
		...
		{
			var text = GameLanguage.getLangByKey("L_A_172");
		...
		private function onBossNoFood():void
		{
			var text = GameLanguage.getLangByKey("L_A_85025");
			XAlert.showAlert(text, Handler.create(this, function() {
				// 食物不足打开商店
				XFacade.instance.openModule(ModuleName.StoreView, [2]);
			}),null,true,false);
