package game.module.login
{
	...
	public class PreLoadingView extends BaseView
	{
		...
		{	
			var loginArr:Array = [
				GameSetting.Login_UID,
				GameSetting.Login_Token,
				isMobile,
				m_systemStr,
				GameSetting.Login_UDID,
				GameSetting.TimeZone,
				GameSetting.Time,
				GameSetting.tag,
		...
		private function setLog(index:int):void {
			var xhr:HttpRequest = new HttpRequest();
			var str:String = "";
			if( GameSetting.isApp )
			{
...
//			}
//			
//			var data = 'type=cflash&typeid=1&version=1.0&sub=1&step='+ GameSetting.Login_UID + str + index +'&ouid=3&platform=4&serverid=5&' +
//				'gameid=9&timestamp='+ TimeUtil.getCurrentTimeStr() +'&timezone=8'+'&newStep='+index+'&openId='+GameSetting.Login_UID + '&newplf=' + str;
			var data = 'type=cflash&typeid=1&version=1.0&sub=1&step=' + index+'&ouid='+GameSetting.Login_UID+'&platform='+str+'&serverid=5&' +
				'gameid=9&timestamp='+ Laya.timer.currTimer +'&timezone=8';
			if(GameSetting.IsRelease||GameSetting.isApp){
				xhr.send("https://upload.mutantbox.com/push_platform.php?" + data, "", "get", "text");
			}
			else{
				xhr.send("http://testupload.movemama.com/push_platform.php?" + data, "", "get", "text");
			}
		}
		
		public function setIndexLog(key:String):void {
			var xhr:HttpRequest = new HttpRequest();
			var str:String = "";
			if( GameSetting.isApp )
			{
			...
			var data = 'type=cflash&typeid=1&version=1.0&sub=1&step=' + key+'&ouid='+GameSetting.Login_UID+'&platform='+str+'&serverid=5&' +
				'gameid=9&timestamp='+ Laya.timer.currTimer +'&timezone=8';
			if(GameSetting.IsRelease||GameSetting.isApp){
				xhr.send("https://upload.mutantbox.com/push_platform.php?" + data, "", "get", "text");
			}
...
//			var data = 'type=cflash&typeid=1&version=1.0&sub=1&step=' + GameSetting.Login_UID + str + key +'&ouid=3&platform=4&serverid=5&' +
//				'gameid=9&timestamp='+ Laya.timer.currTimer +'&timezone=8&newStep='+key+'&openId='+GameSetting.Login_UID + '&newplf=' + str;
...
//			}
		...
		private function connectSrv()
		{
			...
			if(GameSetting.isApp){
				...
				GlobalRoleDataManger.instance.userLogin([
					uid,
					GameSetting.Login_Token,
					isMobile,
					m_systemStr,
