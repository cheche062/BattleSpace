package game.module.mineFight 
{
	...
	public class MineDetail extends BaseView
	{
		...
		private var _mineItem:midMineItemUI;
		
		private var _headIcon:Image;
		// 保护中的小标志
		private var _headProtect:Image;
...
//			trace("矿场数据", value);
			...
			_mineItem.dom_headbg.skin = "mineFight/headBg.png";
			
			if (_headProtect) {
				_mineItem.headContainer.removeChild(_headProtect)
			}
			
			// 有人占领
			if (_detailInfo.uid) {
				var isSelf:Boolean = _detailInfo.uid == User.getInstance().uid;
				_mineItem.dom_headbg.skin = isSelf ? "mineFight/headBg3.png" : "mineFight/headBg2.png";
				if (MineFightScence._pTime > 0) {
					var protectBg = isSelf ? "mineFight/dunp.png" : "mineFight/s_2.png";
					_headProtect = _headProtect || new Image(protectBg);
					_headProtect.pos(-3, -3);
					_mineItem.headContainer.addChild(_headProtect);
				}
			}
