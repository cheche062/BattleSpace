...
//				WebSocketNetService.instance.sendData(ServiceConst.MINE_INIT, []);
		...
		/**占矿TIP红点*/
		private function setBtn_rt_log(bool:Boolean):void{
			view.btn_rt_log.visible = bool;
		}
		...
		private function serviceResultHandler(cmd:int, ...args):void
		{
			...
			switch(cmd)
			{
				case ServiceConst.MINE_FIGHT_LOG:
					if(args[1].reports && args[1].reports.length != 0){
						setBtn_rt_log(true);
					}
					else{
						setBtn_rt_log(false);
...
//				WebSocketNetService.instance.sendData(ServiceConst.MINE_REST_INFO, []);
		...
		public function add_MINE_FIGHT_LOG():void
		{
			Signal.intance.on(ServiceConst.getServerEventKey(ServiceConst.MINE_FIGHT_LOG), this, serviceResultHandler, [ServiceConst.MINE_FIGHT_LOG]);
		}
		
		public function remove_MINE_FIGHT_LOG():void
		{
			Signal.intance.off(ServiceConst.getServerEventKey(ServiceConst.MINE_FIGHT_LOG), this, serviceResultHandler);
		}
