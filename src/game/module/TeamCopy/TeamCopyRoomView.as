...
//					l_teamCopyUnitVo.unit_list=l_obj.unit_list;
		...
		private function onRefStartBtn():void
		{
			isCanStart = true;
			for(var i=0;i<m_teamCopyRoomVo.teamList.length;i++){
				var state = m_teamCopyRoomVo.teamList[i].state;
				if(state == 0){
					isCanStart = false;
				}
			}
			if(!isCanStart ){
				view.StartBtn.gray = true;
			}
			else{
				view.StartBtn.gray = false;
			}
		}
