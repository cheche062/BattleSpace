...
package game.module.fighting.panel
{
	...
	public class FightResultPanel extends BaseDialog
	{
		...
		override public function show(...args):void{
			...
			if( User.getInstance().curGuideArr[0] != undefined && 
				User.getInstance().curGuideArr[0] != null &&
				User.getInstance().curGuideArr[0] != 0 )
			{
				var obj:Object = GameConfigManager.common_guide_vec[GameConfigManager.fun_open_vec[User.getInstance().curGuideArr[0]].g_id];
				if (User.getInstance().curGuideArr.length > 0 && obj &&
		...
		override public function close():void{
			...
			if (User.getInstance().firstRecharge) {
				XFacade.instance.openModule(ModuleName.FirstChargeView,0);
				User.getInstance().firstRecharge = false;
			}
		}
		...
		public function getCloseBtn():Sprite
		{
			return _frview.closeBtn;
		}
