...
package game.module.fighting.mgr
{
	...
	public class FightingManager
	{
		...
		public function getSquad(type:Number = 0 , d:* = null , backH:Handler = null):void{
			...
			{
				serverConst = ServiceConst.PVP_LINEUP;
				WebSocketNetService.instance.sendData(serverConst,[]);
			}
			else
			{
...
//					(heroList[i] as ArmyData).state = i+1;
		...
		public function saveFormation():void{
			XTip.showTip(GameLanguage.getLangByKey("L_A_20517"));
		}
		...
		public function sendStart(data:*=null):void{
			...
			{
				serverConst = ServiceConst.PVP_SAVELINEUP;
				saveFormation();
			}
		...
		public function sendStartBack(... args):void{
			...
			if(_fightingType == FIGHTINGTYPE_SET_HOME
				...
				|| _fightingType == FIGHTINGTYPE_WORLD_BOSS
				|| _fightingType == FIGHTINGTYPE_PVP
			)
			{
...
//				this,sendStartPvpAllOkBack);
		...
		public function backFun():void
		{
			...
			if(_fightingType == FIGHTINGTYPE_ORDINARY)  //组队战斗
			{
				//组队战斗的返回，清除战报，强制关闭战斗
				moveFData.reports = {}; //清除战报
				_scence.fightingView.isFightStart = false;  //关闭自动托管录像
		...
		private function fightingResultBack(v:uint):void
		{
			...
			if(v == AlertType.RETURN_YES)
			{
				...
				if(_scence.completeHandler)
				{
					...
					//战斗结束清理场景数据
					if(_scence && _scence.fightingView &&!_scence.fightingView.isFightStart){
						SceneManager.intance.setCurrentScene(null);
					}
				}else{
					SceneManager.intance.setCurrentScene(SceneType.M_SCENE_HOME);
		...
		public function fightingDataBack(... args):void{
			...
			switch(args[0])
			{
				...
				case ServiceConst.FIGHTING_RESULTS_CONST:
				{
					...
					// 首冲弹层
					if (obj[User.getInstance().uid] && obj[User.getInstance().uid]["firstRecharge"] == 1) {
//						XFacade.instance.openModule(ModuleName.FirstChargeView,0);
						User.getInstance().firstRecharge = true;
					}
					
					var uid:* = GlobalRoleDataManger.instance.userid;
					if(obj && obj.hasOwnProperty(uid))
					{
						_overData = new FightingResultsData();
						_overData.setData(obj[uid]);
...
//					}
					...
					if(obj && obj.hasOwnProperty(uid))
					{
						var _overData2:FightingResultsData = new FightingResultsData();
						_overData2.setData(obj[uid]);
						//测试数据
...
//						];
		...
		// pvp副本的处理
		public function showPvpFightReport(t_fData:Object , t_rData:Object = null , overH:Handler = null):void
		{
			_overH = overH; 
			_fightingType = 0;
			showFightReportBack(t_fData,t_rData, false);
		}
