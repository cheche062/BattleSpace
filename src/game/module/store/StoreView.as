package game.module.store
{
	...
	public class StoreView extends BaseDialog
	{
					...
					for(var i = 0;i<activityData.length;i++){
						if(activityData[i].tid == 19){
							...
							clearTimerHandler = ToolFunc.limitHandler(Math.abs(activityData[i].end_date_time - TimeUtil.nowServerTime), function(time) {
								var detailTime = time;
								view.lbCountdown.text =  GameLanguage.getLangByKey('L_A_83082') +' '+TimeUtil.getTimeCountDownStr(detailTime,false);
							}, function() {
		...
		private function onRender(cell:Box,index:int):void
		{
			...
			var numTf_base:Text = cell.getChildByName("numTf_base") as Text;
			var lbDiscount:Label = cell.getChildByName("lbDiscount") as Label;
			var imgDiscount:Image = cell.getChildByName("imgDiscount") as Image;
			...
			lbDiscount.text = data['discount']+'%OFF';
			...
			num.text = pArr[1];
			cell.addChildAt(item,cell.numChildren-5);
