package game.module.chests
{
	...
	public class ChestMainCardItem extends Box
	{
		...
		public function doClearTimerHandler():void {
...
//			Laya.timer.clearAll(this);
		...
		private function setCardCost()
		{
			...
			// 打折 (且不是紫色抽卡道具)
			if(m_data.exploration_sale.length !=0 &&Number(m_data.exploration_sale[this.m_type].discount)!=100&& Number(m_data.exploration_sale[this.m_type].discount)!=0 && m_data.template_info.end_date_time>TimeUtil.nowServerTime)
			{
				...
				//打折的
				m_ui.TenTimesCostTxt.text=XUtils.formatResWith(l_arr[1]*(100-discount)/100)
				//				}
...
//					var detailTime = TimeUtil.toDetailTime(time);
					var detailTime = time;
					m_ui.OffTenTimeText.text = TimeUtil.getTimeCountDownStr(detailTime,false);
					m_ui.lbTipDiscount.text = discount + '% OFF ' + TimeUtil.getTimeCountDownStr(detailTime,false);
//					trace('倒计时jinxing：：：');
				}, function() {
					m_ui.OffTenTimeText.text = "";
					m_ui.lbTipDiscount.text = '';
...
//					setCardCost();
				...
				m_ui.imgTipDiscount.visible = true;
