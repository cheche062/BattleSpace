package game.module.armyGroup
{
	...
	public class ArmyGroupMapView extends BaseView
	{
		...
		/**所有有的星球信息*/
		public static var all_planet_data:Object
		...
		/**国战每秒恢复体力*/
		private var War_Recovery:Number = 0;
...
//			_ufoContainer.graphics.drawRect(0, 0, 200, 100, "#f00");
		...
		private function createAllBigMap():void {
			...
			// 稍微扩大一点地图以便边缘的星球展示完整
			big_map.size(getMaxPosValue("x", "width") + 300, getMaxPosValue("y", "height") + 700);
		...
		private function cteateFog():void {
			var data_city = all_planet_data = ResourceManager.instance.getResByURL("config/juntuan/juntuan_city.json");
			// x y 方向上的个数
		...
		private function serviceResultHandler(... args):void {
			...
			switch (args[0]) {
				...
				case ServiceConst.BOSS_OPEN_VIEW:
					...
					clearTimerHandler = ToolFunc.limitHandler(Math.abs(endTime - TimeUtil.nowServerTime), function(time) {
						...
						if(time>60*5){
							...
							view.lb_boss_time.text = TimeUtil.formatStopwatch(time*1000);
							view.lb_boss_time.visible = true;
						}
					}, function() {
						view.lb_boss_time.text = '';
						view.lb_boss_time.visible = false;
				...
				case ServiceConst.ARMY_GROUP_MAP_INIT:
					War_Recovery = Number(args[1].add_muscle_second);
					User.getInstance().War_Recovery = War_Recovery;
...
//					bossAni.visible = !!boss_coord;
					...
					CURRENT_SEASON = Number(args[1].season_id) + 1;
...
//					view.getRewardArea.visible = Boolean(parseInt(_nowFightState) == 3);
		...
		{
			var colorGray:String = "#999";
			view.ftTxt.innerHTML = joinLabelString("L_A_21012", "fight_begin", "fight_end", colorGray, colorGray);
			view.jsTxt.innerHTML = joinLabelString("L_A_21013", "result_begin", "result_end", colorGray, colorGray);
			view.ljTxt.innerHTML = joinLabelString("L_A_21014", "reward_begin", "reward_end", colorGray, colorGray);
...
//						view.getRewardArea.visible = Boolean(parseInt(_nowFightState) == 3);
		...
		override public function addEvent():void {
			...
			Signal.intance.on(ServiceConst.getServerEventKey(ServiceConst.BOSS_OPEN_VIEW), this, serviceResultHandler);
		...
		override public function removeEvent():void {
			...
			big_map && big_map.off(Event.MOUSE_DOWN, this, this.mapStarDropHandler);
			big_map && big_map.off(Event.MOUSE_UP, this, this.mapStopDropHandler);
			
			allButtons.forEach(function(item) {
				item && item.offAll(Event.CLICK);
			}, this);
			...
			Signal.intance.off(ServiceConst.getServerEventKey(ServiceConst.BOSS_OPEN_VIEW), this, serviceResultHandler);
