...
//					CURRENT_ACT_ID = "-1";
					...
					// 另外后添加进来的两个活动
					if (args[1].activity3) {
						if (args[1].activity3["giftbag4newplayer"] == 1) {
							_actList.push({id:"giftbag4newplayer", tid: 21});
						}
						if (args[1].activity3["timelimitedtask"] == 1) {
							_actList.push({id: "timelimitedtask" , tid: 22});
						}
						if (args[1].activity3["levelGift"] == 1) {
							_actList.push({id:"levelGift", tid: 23});
						}
...
//							trace("老虎机");
						...
						// 后添加进来的三个
						case 21://限时礼包
							actViewPool[_actList[i].id] = actViewPool[_actList[i].id] || new TimeLimitView();
							break;
						case 22://限时任务
							actViewPool[_actList[i].id] = actViewPool[_actList[i].id] || new TimelimitedtaskView();
							break;
						case 23://等级礼包
							actViewPool[_actList[i].id] = actViewPool[_actList[i].id] || new LevelGiftView();
							break;
...
//								actViewPool[CURRENT_ACT_ID].y = (view.viewContainer.height - actViewPool[CURRENT_ACT_ID].height) / 2;
		...
		private function onClose():void
		{
			...
			for (var key in actViewPool) {
				if (actViewPool[key] && actViewPool[key].destroy) {
					actViewPool[key].destroy();
				}
			}
			actViewPool = {};
		}
		...
		public function get view():ActivityMainViewUI{
			return _view;
		}
