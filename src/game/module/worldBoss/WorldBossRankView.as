package game.module.worldBoss
{
	...
	public class WorldBossRankView extends BaseDialog
	{
		...
		private function format(data:Object):void{
			...
			this.view.tfKill.text = data.myKill+"";
		...
		private function showPage(index:int):void{
			...
			var type:String = (_bossId+"").charAt(0) || "1";
			var array:Array = getAllRankData(type);
			for (var k:int = 0; k < array.length; k++) 
			{
				if( !_rankData[k] )
				{
					_rankData.push(array[k]);
				}
			}
		...
		private function getAllRankData(type:String):Array
		{
			var tempList:Array = [];
			for(var i:String in rankConfig)
			{
				if(rankConfig[i].type == type)
				{
					tempList.push(rankConfig[i]);
				}
			}
			return tempList;
		}
		...
		private function onClick(e:Event):void
		{
			switch (e.target)
			{
				case view.btnClose:
					...
					break;
				default:
					var arr:Array = (e.target.name+"").split("_");
					if(arr[0] == "WB"){
						ItemTips.showTip(arr[1]);
					}
					break;
			}
		}
		...
		override public function createUI():void
		{
			...
			var result:Object = ResourceManager.instance.getResByURL("config/p_boss/p_boss_param.json");
			var str:String = GameLanguage.getLangByKey("L_A_85055");
			str = str.replace(/{(\d+)}/, result[26].value);
			view.tfRankTip.text =  str;
		}
