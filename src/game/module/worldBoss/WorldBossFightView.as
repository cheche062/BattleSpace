...
//				}
		...
		/**倒计时的清理函数*/
		private var clearTimerHandler_2:Function;
		private function doClearTimerHandler():void {
			clearTimerHandler_2 && clearTimerHandler_2();
		...
		private function onServerResult(...args):void{
			...
			switch(cmd) {
				...
				case ServiceConst.BOSS_OPEN_VIEW:
					var result = args[1];
					var endTime_boss = result["end_time"];
					doClearTimerHandler();
					clearTimerHandler_2 = ToolFunc.limitHandler(Math.abs(endTime_boss - TimeUtil.nowServerTime), function(time) {
						//						var detailTime = TimeUtil.toDetailTime(time);
						if(time>60*5){
							...
							view.img_boss_time.visible = true;
							view.img_red_bg.visible = true;
						}
...
//					dieInfoCollection.push(args[1]);
				...
				case ServiceConst.BOSS_AUTO:
					...
					break;
				/**BOSS界面任务，当杀敌数满，可以领取奖励时，MISSION的ICON的红点展示*/
		...
		private function setDataStatus(args:Object):void
		{
			...
			if(not_get_num == 0){
				view.img_rt_mission.visible = false;
			}
			else{
				view.img_rt_mission.visible = true;
			}
		}
		// 列表数据源
		...
		override public function addEvent():void
		{
			...
			Signal.intance.on(ServiceConst.getServerEventKey(ServiceConst.BOSS_MISSION_INIT), this, onServerResult);
...
//			}
		...
		override public function removeEvent():void {
			...
			Signal.intance.off(ServiceConst.getServerEventKey(ServiceConst.BOSS_MISSION_INIT), this, onServerResult);
