	...
	public class NewPataView extends BaseDialog
	{
		...
		private function playEndAni():void
		{
			if(NewPataData.intance.passId==30)
			{
				...
				end.loadAtlas("appRes/effects/endEffect.json");
				end.play(0,false);
		...
		private function playBoomAni(reward:Array):void
		{
			...
			boom.loadAtlas("appRes/effects/baokai.json");
		...
		override public function show(...args):void
		{
			...
			//					trace("cd.name:"+cd.name);
			sel.loadAtlas("appRes/effects/selEffect.json");
			sel.play(0,false);
		...
		//扫荡所需的道具ID
		private var sweepingNeedId = 1;
		//扫荡所需的道具数量
		private var sweepingNeedNum = 0;
		...
		private function setTimes():void
		{
			...
			for(var key:String in levelObj)
			{
				...
				if(id1 == curId)
				{
					...
					c_name = id1 +'.'+GameLanguage.getLangByKey(levelObj[key]["stage_name"]);
				}else if(curId>30)
				{
					c_name = 30 +'.'+GameLanguage.getLangByKey(levelObj["30"]["stage_name"]); 
				}
				if(id1 == maxId)
				{
			...
			}else
			{
				for(var key:String in levelObj)
				{
					...
					{
						//					trace("stage_name:"+levelObj[key]["stage_name"]);
						//						toSweepName = GameLanguage.getLangByKey(levelObj[key]["stage_name"]);
						toSweepName = 'Step.' + id1;
					}
				}
				if(toSweepName == "")
				{
			...
			view.sweepNum.text = GameLanguage.getLangByKey("L_A_88144")+toSweepName;
...
//						boxEff.y = 578 + (view[na2] as Image).displayHeight/2;
		...
		override public function onStageResize():void
		{
			...
			view.dom_bottom.y = view.height - view.dom_bottom.height - 30;
			view.map.y = (view.height - view.map.height) / 2;
