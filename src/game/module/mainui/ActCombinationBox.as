...
//			return;
		...
		//过掉新手任务的玩家，每次主动打开游戏，都会主动弹出 hot events ,等级大于5J
		private function openFirstActivity():void{
			if(!User.getInstance().hasOpenHotEvents && User.getInstance().hasFinishGuide && User.getInstance().level >=5 && 
				User.getInstance().pop_hotevents == 1){
				User.getInstance().hasOpenHotEvents = true;
				XFacade.instance.openModule(ModuleName.ActivityMainView);
			}
		}
