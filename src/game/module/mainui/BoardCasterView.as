package game.module.mainui 
{
	...
	public class BoardCasterView extends BaseView 
	{
		...
		{
			var str:String;
			//跑马灯的入口只有一个，要改变跑马灯的颜色，建议在读取内容的时候区分修改
			view.infoTxt.color = "#c3e0fe";
			view.imgBg.skin = "mainUi/bg_paomadeng.png";
			switch(parseInt(GlobalRoleDataManger.instance.boardcastVec[0].type))
			{
				...
				// 国战BOSS开启通知
				case 3:
					view.infoTxt.color = "#ffef87";
					view.imgBg.skin = "mainUi/bg_paomadeng_1.png";
					str = GameConfigManager.boardcastVec[3].text;
					break;
				//红色品质英雄
				case 8:
					str = GameLanguage.getLangByKey(GameConfigManager.boardcastVec[8].text)
					.replace("{0}", GlobalRoleDataManger.instance.boardcastVec[0].username);
					var id = GlobalRoleDataManger.instance.boardcastVec[0].sysMsg[0];
					var name = GameLanguage.getLangByKey(GameConfigManager.items_dic[id]["name"]);
					str = str.replace("{1}", name);
					break;
				//红色品质装备
				case 9:
					str = GameLanguage.getLangByKey(GameConfigManager.boardcastVec[9].text)
					.replace("{0}", GlobalRoleDataManger.instance.boardcastVec[0].username);
					var id = GlobalRoleDataManger.instance.boardcastVec[0].sysMsg[0];
					var name = GameLanguage.getLangByKey(GameConfigManager.items_dic[id]["name"]);
					str = str.replace("{1}", name);
