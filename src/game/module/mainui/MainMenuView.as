				...
				case MainMenuView.MENU_LVUP:
					...
					if(bvo.building_type == DBBuilding.TYPE_FUN){
						if(bvo.building_id == DBBuilding.B_PROTECT){
							...
							arr.push(view.jfBtn,view.shop1Btn, view.mineBtn);
...
//					this.view.enterBtn.disabled = true;
		...
		{
			var item:BuildingItem = view.pane.content.getChildAt(0) as BuildingItem;
			return item.localToGlobal(new Point(view.x + view.pane.content.x + item.x , view.y + view.pane.content.y + item.y));
		}
		
		public function getBuilding():Sprite
		{
			var item:BuildingItem = view.pane.content.getChildAt(0) as BuildingItem;
			if( !item )
			{
				return view.pane.content;
			}
			return item;
		}
