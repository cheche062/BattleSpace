package game.module.camp
{
	...
	public class NewUnitInfoOperView extends BaseChapetrView
	{
		...
		private function bindAmData():void
		{
			...
			_starLv.starColor(Number(selectAmData.unitVo.rarity)-1);
...
//			trace('上限：', _limitSkillList)
		...
		private function getCurrentSkills(data):Array {
			...
			if (data["skillId2"]) {
				var skill2s:Array = data["skillId2"].split("|").map(function(item) {
					//被动技能的来源英雄列表，和这个英雄匹配不上，说明是外来技能，不显示
					var data_config = ResourceManager.instance.getResByURL("config/unit_skill2.json");
					var skill_source = data_config[item].skill_source.split("|");
					for(var i =0; i<skill_source.length;i++){
						if(skill_source[i] == data["unitId"]){
							skills.push([item, 2, true]);
						}
					}
//					return [item, 2, true];
				});
