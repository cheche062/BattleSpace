package game.module.grassShip
{
	 ...
	 * 2018-01-25
	 */
	public class GrassShipView extends BaseView
	{
		/**阶段奖励   数据*/
		private var jiejian_stage_reward:Object;		
		/**购买次数   数据*/
		private var jiejian_time:Object;		
		/**大本营等级对应的阶段奖励倍率   数据*/
		private var jiejian_rparam:Object;		
		
		
		/**指针间距*/
		private const POINTER_SPACE = 82 + 30; 
		/**起始y轴坐标*/
		private const POINTER_POS_0 = 336;
		...
		public function GrassShipView(data:Object)
		{
			super();
			info = data;
			this.width = 713;
			this.height = 521;
			ResourceManager.instance.load(ModuleName.GrassShipView, Handler.create(this, resLoader));
		}
...
//			不可领取的突破奖励档位
		...
		private function updateJieduan(data:GrassShipServerVo):void{
			...
			for (var i = 0; i < view.dom_VBox_stage.numChildren; i++) {
				...
				child.dataSource = ToolFunc.copyDataSource(child.dataSource, result);
				child.gray = result["gray"];
			}
			
			/**箱子数据*/
			var box_data = jiejian_stage_reward;
...
/*		private function createDaojuItem():void{
			...
			switch(event.target){
				...
				case view.btn_add:
					...
					XFacade.instance.openModule("ItemAlertView", [text, priceArr[0], priceArr[1], function(){
						sendData(ServiceConst.CAOCHUAN_BUY_TIMES, [info.id]);
					}]);
		...
		private function boxClickHandler(event:Event):void{
			...
			sendData(ServiceConst.CAOCHUAN_DAY_REWARD, [info.id, Number(dataSource["dom_text"])]);
		}
		...
		private function onResult(...args):void{
			...
			switch(args[0]){
				...
				case ServiceConst.CAOCHUAN_GET_INFO:
					server_data = args[1]["my_info"];
					
					jiejian_rparam = args[1]["jiejian_rparam"];
					jiejian_stage_reward = args[1]["jiejian_stage_reward"];
					jiejian_time = args[1]["jiejian_time"];
					
					createItemBox();
					
					updateJieduan(server_data);
					updateAttempts(server_data);
		...
		public function addToStageEvent():void{
			...
			// 进入界面
			sendData(ServiceConst.CAOCHUAN_GET_INFO, [info.id]);
		}
