package game.module.guild
{
	 ...
	 * 
	 */
	public class GuildChatViewNew extends BaseView
	{
		...
		public function GuildChatViewNew()
		{
			super();
		...
		override public function show():void
		{
			...
			this.x = INIT_X;
			toggleState(false);
			toggleRedot(User.getInstance().hasNewChat);
			
			
			view.tabCtrl.labels = "GUILD";
			view.tabCtrl.x=152;
			
			UIRegisteredMgr.AddUI(view.closeBtn, "GuildChat");
		}
...
//			var chatList:Array = view.tabCtrl.selectedIndex == 0 ? worldChatList : gonghuiChatList;
			var chatList:Array = gonghuiChatList;
		...
		private function onClickHandler(e:Event):void
		{
			switch (e.target)
			{
				case view.closeBtn:
					...
					User.getInstance().hasNewChat = false;
					toggleRedot(false);
					if (isShow) {
						toggleState(isShow);
