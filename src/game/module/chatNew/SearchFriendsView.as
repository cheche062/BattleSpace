package game.module.chatNew
{
	...
	public class SearchFriendsView extends BaseDialog
	{
		...
		override public function show(... args):void {
			...
			// App端没有facebook的好友邀请
			if (Browser.onIOS || Browser.onAndriod) {
...
//				view.dom_tab.labels = "L_A_3025,L_A_3024,L_A_3026";
				...
				case 2:
					...
					if (!GameSetting.isApp) {
						function initData(arr:Array = [] ):void {
							...
							// 截取10个 防止太多
							var _random_arr:Array = randomDataFromList(arr, 10);
							var uidArr:Array = _random_arr.map(function(item) {
								return item["uid"];
							});
						...
						view.dom_tips.visible = false;
						view.btn_sq.visible = false;
					}
					else
					{
						view.dom_tips.visible = true;
						view.btn_sq.visible = true;
						
						view.dom_tips.text = GameLanguage.getLangByKey("L_A_3034");
						view.btn_sq.label = GameLanguage.getLangByKey("L_A_20946");
					}
					view.dom_facebookList.array = null;
		...
		/**在数据里随机取值*/
		private function randomDataFromList(data:Array, len:int = 3):Array {
		...
		private function onClickHandler(e:Event):void {
			switch (e.target) {
				...
				case view.btn_facebook:
					...
					break;
				case view.btn_sq:
					AndroidPlatform.instance.FGM_GameRequest(changeTextValue);
//					view.dom_tips.visible = false;
//					view.btn_sq.visible = false;
					
//					AndroidPlatform.instance.FGM_getFacebookAppFriends(Handler.create(this, function(fb_data:Object) {
//						if (Browser.onIOS) {
//							trace("ios FB好友数据:::::=============", fb_data);
//							fb_data = JSON.parse(fb_data);
//							if (!fb_data || !fb_data.result || !fb_data.result.data) return;
//							
//							// 截取10个 防止太多
//							var _random_arr:Array = randomDataFromList(fb_data.result.data, 10);
//							var uidArr:Array = _random_arr.map(function(item) {
//								return item["id"];
//							});
//							var result:String = uidArr.join(",");
//							this.sendData(ServiceConst.DEMAND_PLAYER_INFO, [result]);
//							
//							
//							
//						} else if (Browser.onAndriod) {
//							trace("安卓FB好友数据:::::=============", fb_data);
//							if (!fb_data || !fb_data.data) return; 
//							// 截取10个 防止太多
//							var _random_arr:Array = randomDataFromList(fb_data.data, 10);
//							var uidArr:Array = _random_arr.map(function(item) {
//								return item["id"];
//							});
//							var result:String = uidArr.join(",");
...
//					}));
		...
		private function changeTextValue():void
		{
			view.dom_tips.text = GameLanguage.getLangByKey("L_A_3042");
			view.btn_sq.label = GameLanguage.getLangByKey("L_A_3043");
		}
