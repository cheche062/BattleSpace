...
//			refreshRewardNum();
		...
		public function getIntegralReward(id:int):void
		{
			Signal.intance.on(
				ServiceConst.getServerEventKey(ServiceConst.PVP_GETINTEGRAL),
				this,getIntegralBack);
			WebSocketNetService.instance.sendData(ServiceConst.PVP_GETINTEGRAL,[id]);
		}
		
		public function getIntegralBack(... args):void
		{
			WebSocketNetService.instance.sendData(ServiceConst.PVP_GETINFO,[]);
			Signal.intance.off(
				ServiceConst.getServerEventKey(args[0]),
...
//			getedRewards.push(num);
			...
			pvpProxy.isGetIntegral = true;
			var targetData:Object = pvpProxy.getIntegralRewardData(id);
			...
			var childList:Array = arr.map(function(item:String, index){
		...
		public function getReward():void
		{
			Signal.intance.on(
				...
				this,getRewardBack);
			WebSocketNetService.instance.sendData(ServiceConst.PVP_GETREWARD,[]);
		}
		
		public function getRewardBack(... args):void
		{
			var pvpProxy:PvpProxy = Facade.getInstance().retrieveProxy("PvpProxy") as PvpProxy;
			pvpProxy.isGetRank = true;
...
//			refreshRewardNum();
			...
			var pvpProxy:PvpProxy = Facade.getInstance().retrieveProxy("PvpProxy") as PvpProxy;
			var targetData:Object = pvpProxy.getRankRewardData(pvpProxy.rank);
			if( !targetData ) return;
			var arr:Array = [];
			var reward:String = targetData["reward"];
			if(reward.indexOf("|") != -1)
		...
		public function getShopCountBySid(sId:Number):int
		{
...
//				XFacade.instance.openModule(ModuleName.ShowRewardPanel,[vo.showItems]);
		 ...
		 *根据积分获取段位等级对象
		 */
		public function getPvpLevelByIntegral(integral:Number):PvpLevelVo
		{
