package game.module.liangjiu
{
	...
	public class RonglianView extends ronglianUI
	{
		...
		/**是否自动玩中*/
		public static var isAutoPlay:Boolean;
		/**是否自动玩中*/
		private var tech_point:int = null;
		/**升级动画*/
		private var animateDom:Animation;
		/**动画资源*/
		private var aniUrl:String = "appRes/effects/ronglian.json";
...
//			btn_confirm.disabled = true;
			...
			UIRegisteredMgr.AddUI(this.processBox,"$ronglian_process");
		}
		
		public function show(data):void {
			dom_ten.clickHandler = new Handler(this, checkBoxSelectHandler, [5]);
		...
		public function update():void {
			...
			if (tech_point == null || tech_point == state.tech_point) {
				...
				if (!animateDom) {
					...
					animateDom.loadAtlas(aniUrl, Handler.create(this, playAnimation));
				}
...
//			btn_confirm.disabled = options.indexOf(itemStr) == -1;
		...
		private function renderJingshi(data:Array):void {
			dom_list.array = state.getJingshiDataArray(data);
		}
		
		/**晶石皮肤*/
		private function getJingshiSkin(id:int):String{
			return state.getJingshiSkin(id);
		}
		
		private function checkBoxSelectHandler(num:int):void {
			var current_dom:CheckBox = num == 5 ? dom_ten : dom_hundred;
			var other_dom:CheckBox = num == 5 ? dom_hundred : dom_ten;
		...
		override public function destroy(destroyChild:Boolean = true):void {
			...
			UIRegisteredMgr.DelUi("doubleTips");
			UIRegisteredMgr.DelUi("mutilTimes");
			UIRegisteredMgr.DelUi("startRLBtn");
			UIRegisteredMgr.DelUi("ronglianResult");
			UIRegisteredMgr.DelUi("$ronglian_process");
		}
