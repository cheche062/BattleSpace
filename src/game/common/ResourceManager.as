package game.common
{
	...
	{	
		...
		/***正在使用的配置表以及资源json引用计数*/
		public var inUseingResource:Object = {};
...
//				trace("result[0]:"+result[0]);
		...
		/**增加资源的引用计数*/
		private function addInUseingCount(data:Array, name):void {
			data.forEach(function(item) {
				if (inUseingResource[item.url]) {
					if (typeof (inUseingResource[item.url]) == "string" && inUseingResource[item.url] != name) {
						inUseingResource[item.url] = [name].concat(inUseingResource[item.url]);
					} else if (ToolFunc.isArray(inUseingResource[item.url]) && inUseingResource[item.url].indexOf(name) == -1) {
						inUseingResource[item.url] = [name].concat(inUseingResource[item.url]);
					}
				} else {
					inUseingResource[item.url] = name; 
				}
			});
//			trace("使用中的资源。。。。", inUseingResource);
		}
		...
		public function clearResource(name:String):void {
			...
			urlArr.forEach(function(item) {
				if (inUseingResource[item.url]) {
					if (typeof (inUseingResource[item.url]) == "string") {
						Laya.loader.clearRes(item.url);
						
						delete inUseingResource[item.url];
						
					} else if (ToolFunc.isArray(inUseingResource[item.url])) {
						var _index = inUseingResource[item.url].indexOf(name);
						if (_index > -1) {
							inUseingResource[item.url].splice(_index, 1);
							if (inUseingResource[item.url].length == 1) {
								inUseingResource[item.url] = inUseingResource[item.url][0]; 
							}
						}
					}
