package game.common
{
	...
	public class XUtils
	{
		...
		/**
		 * 部分商店名字太长
		 * 这个名称的距离一共可以支持17个字符，如果道具字符＞17，则只显示前14个字符，结尾用“..."代替，如果字符＜=17，则显示全部字符 
		 * */
		public static function strLengthLimit(name:String):String{
			var str:String = GameLanguage.getLangByKey(name);
			if(str.length>17){
				str = str.slice(0,13) + '...';
			}
			return str;
		...
		public static function createEllipse(sp:Graphics, x:int, y:int, a:int, b:int, stepScale:int=1, color:String='#ffffff'):Array
		{
			var points:Array=[];
			var step:int=(a > b) ? stepScale / a : stepScale / b;
