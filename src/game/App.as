...
//					trace("itemId");
		...
		private function onNewFunOpen(... args):void
		{
			...
			if (User.getInstance().curGuideArr.length == 1 && LayerManager.instence.isOnlyMain && User.getInstance().isInMainView)
			{
				if (GameConfigManager.fun_open_vec[args[1][0]].lx == 2)
				{
					...
					var obj:Object = GameConfigManager.common_guide_vec[GameConfigManager.fun_open_vec[args[1][0]].g_id];
					if (obj && obj.hasOwnProperty("special") && obj.special == "waitFight")
					{
						return;
					}
		...
		private function fightingDataBack2New(... args):void {
			var obj:Object = args[1];
//			obj[2].firstRecharge = 1
			// 首冲弹层
		...
		private function initEvent():void
		{
			...
			Signal.intance.on(ServiceConst.getServerEventKey(ServiceConst.FIGHTING_RESULTS_CONST2), this,fightingDataBack2New);
